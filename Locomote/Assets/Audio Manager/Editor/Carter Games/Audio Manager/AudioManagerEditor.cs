...
namespace CarterGames.Assets.AudioManager
{
    ...
    public class AudioManagerEditor : Editor
    {
        ...
        private const string PlayIconLocation = "Carter Games/Audio Manager/Play";
        private const string StopIconLocation = "Carter Games/Audio Manager/Stop";
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (!IsScriptingDefinePresent("Use_CGAudioManager_Static", EditorUserBuildSettings.activeBuildTarget))
                    AddScriptingDefine("Use_CGAudioManager_Static", EditorUserBuildSettings.activeBuildTarget);
            }
        ...
        private void FirstSetup()
        {
            ...
            if (!Directory.Exists(Application.dataPath + "/Audio"))
            {
                ...
                file.objectReferenceValue = (AudioManagerFile)AssetDatabase.LoadAssetAtPath("Assets/Audio/Files/Audio Manager File.asset", typeof(AudioManagerFile));
        ...
        private void HeaderDisplay()
        {
            ...
            EditorGUILayout.BeginHorizontal();
            GUILayout.FlexibleSpace();
            EditorGUILayout.LabelField("Version: 2.5.1", GUILayout.Width(TextWidth("Version 2.5.1  ")));
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndHorizontal();
        ...
        private int CheckAmount()
        {
            ...
            if (fileDirs.arraySize > 0)
            {
                ...
                {
                    if (Directory.Exists(Application.dataPath + "/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue))
                    {
                        // 2.3.1 - adds a range so it adds each directory to the asset 1 by 1
                        _allFiles.AddRange(new List<string>(Directory.GetFiles(Application.dataPath + "/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue)));
                    }
                    else
                    {
        ...
        private void AddAudioClips()
        {
            ...
            for (int i = 0; i < fileDirs.arraySize; i++)
            {
                ...
                else if (Directory.Exists(Application.dataPath + "/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue))
                    // 2.3.1 - adds a range so it adds each directory to the asset 1 by 1.
                    _allFiles.AddRange(new List<string>(Directory.GetFiles(Application.dataPath + "/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue)));
                else
                {
                    // !Warning Message - shown in the console should there not be a directory named what the user entered
#if (!CarterGames_Assets_AudioManager_NoDebug)
                    Debug.LogWarning("(*Audio Manager*): Path does not exist! please make sure you spelt your path correctly: " + Application.dataPath + "/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue);
#endif
                    shouldShowMessage = true;
                }
            }
            ...
            if (fileIsPopulated.boolValue)
            {
                if (fileLib.arraySize > 0 && fileLib.arraySize != CheckAmount())
                {
                    ...
                    if (fileDirs.arraySize != 0)
                    {
                        ...
                        for (var i = 0; i < fileDirs.arraySize; i++)
                        {
                            _errorString = _errorString + "assets/audio/" + fileDirs.GetArrayElementAtIndex(i).stringValue + "\n";
                        }
                    }
                    else
                ...
                else if (CheckAmount() == 0)
                {
                    ...
                    if (fileDirs.arraySize != 0)
                    {
                        ...
                        for (int i = 0; i < fileDirs.arraySize; i++)
                        {
                            _errorString = _errorString + "assets/audio/" + fileDirs.GetArrayElementAtIndex(i) + "\n";
                        }
                    }
                    else
                        _errorString = "No clips found in: " + "assets/audio/";
